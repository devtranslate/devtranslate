...
export const paddingSizes = ['none', 'xs', 'xs', 's', 'm', 'l', 'xl', 'xxl', 'xxxl'];
...
A propriedade **hidden** permite configurar até qual breakpoint a coluna ficará escondida.

<Story name="Hidden">
  <Grid paddingTop={{ xs: 'xs' }} paddingBottom={{ xs: 'l' }}>
    <ColumnBorder xxs={12} padding={{ xs: 's' }} hidden={{ xs: false, m: true }}>
      <Text>
        <Text as="b">MOBILE</Text>: Esta coluna fica hidden a partir 1024px ( {`hidden={{ xs: false, m: true }}`} )
      </Text>
    </ColumnBorder>
    <ColumnBorder xxs={12} padding={{ xs: 's' }} hidden={{ xs: true, m: false }}>
      <Text>
        <Text as="b">DESKTOP</Text>: Esta coluna fica hidden até 1024px ( {`hidden={{ xs: true, m: false }}`} )
      </Text>
    </ColumnBorder>
  </Grid>
</Story>
